/*
	...
	cute_sound.h - v1.12 (plus a few changes by Andrew Trevorrow -- see AKT)
	...
	SUMMARY
		...
		An SDL2 implementation of cute_sound is available on platforms where SDL2 is available,
	...
	High-level API
		...
		We create cs_playing_sound_t's indirectly with cs_play_sound_def_t structs.
		cs_play_sound_def_t is a POD struct so feel free to make them straight on the stack.
		cs_make_def sets up initialization parameters. Here's an example to load a wav and
		play it:
...
// The returned struct will contain a null pointer in cs_loaded_sound_t::channels[0]
...
#ifdef __clang__
	//AKT #define CUTE_SOUND_ASSERT_INTERNAL __builtin_trap()
	//AKT get nicer "Assertion failed: ..." info if compiled with -g:
	#define CUTE_SOUND_ASSERT(X) assert(X)
#else
...
cs_context_t* cs_make_context(void* unused, unsigned play_frequency_in_Hz, int buffered_samples, int playing_pool_count, void* user_allocator_ctx)
{
	...
	CUTE_SOUND_CHECK(ret == noErr, "Failed to set stream format");
...
int cs_insert_sound(cs_context_t* ctx, cs_playing_sound_t* sound)
{
	...
	// for a playing sound pool. InsertSound assumes the pool does not exist, and is a part
...
void cs_stop_all_sounds(cs_context_t* ctx)
{
	// This is a part of the high level API, not the low level API.
	...
	while (sound)
	{
		...
		sound->active = 0;
		sound->loaded_sound = 0; //AKT avoids crash in cs_mix (also consistent with cs_stop_sound)
		sound = sound->next;
	}
	cs_unlock(ctx);
}
