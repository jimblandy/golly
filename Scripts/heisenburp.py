...
# Author: Dave Greene, 22 January 2007.
#   Latest changes:  fixed tiling/stacking default problem
#                    added T toggle to switch between stacked and tiled views
#                    included three input glider signals by default
#                    fixed countdown message to display correctly
#                    changed status bar text to display T-toggle only in 1.2
...
def burp():
  ...
  if multilayer:
    ...
    t=""
  instr="Use ENTER or SPACE to run, + or - for speed, arrow keys/mouse in SE tile to view, " \
        + t + "S to send another signal, R to restart, Q to quit. Speed: "
  ...
  while lower(ch)<>"q":
    ...
    if place_signal>0 and run_flag==True:
      g.show("Time to next signal placement: " + str(1847 - ticks + last_signal) + " ticks.")
    else:
    ...
    if ch==" " or run_flag==True:
      ...
      if oldticks<3960 and ticks>=3960: sel_speed=.666667
      if oldticks<6250 and ticks>=6250: sel_speed=0       
      if oldticks<4000 and ticks>=4000: viewport_speed=1.5
...
def show_status_text(s, d, t):
  if d==-1:
    if t==1:
      g.show(s + str(t) + " tick per step.")
    else:
      g.show(s + str(t) + " ticks per step.")
  else:
    g.show(s + str(d) + " sec delay.")
