...
local selx=600
local sely=365
local place_signal=3
local helpstring=[[
Use ENTER and SPACE to run or halt the pattern;
...
local instr="Press H for help. "

-------------------------------------------------------

local function sleep(n) -- n is seconds (with precision in hundredths of a second)
    local t0 = clock()
    while clock() - t0 <= n do
        g.doevent("")   -- no-op, but allows user to abort script here
    end
end

-------------------------------------------------------

local function show_status_text(s, d, t)
    if d==-1 then
        if t==1 then g.show(s.."Speed is "..t.." tick per step.")
        else g.show(s.."Speed is "..t.." ticks per step.")
        end
    else g.show(s.."Delay between ticks is "..d.." seconds.")
    end
end

-------------------------------------------------------

local function prepare_burp()
    while g.numlayers()>1 do g.dellayer() end
    local highway_robber=g.parse([[143bo$143b3o$146bo$145b2o2$126bo42bo$126b3o38b3o$129bo36bo$128b2o11b2o
    ...
    local connecting_neck=g.parse([[24bo$22b3o$21bo$21b2o$6b2o$7bo$7bobo$8b2o10bo$19bobo$19bobo$20bo4b2o$
    ...
    local transmitter2c3=g.parse([[180b2o$180b2o$219bo$219b3o$178b2o42bo$116bo61b2o41b2o$115bobo47b2o99b
    ...
    4bobo$74bo5bo$79b2o!]])
    local head2c3=g.parse("8b2o$3bo2bo2bo$3b6o2$3b6o$2bo6bo$2bo2b5o$obobo$2o2bo$4bo$3b2o!")
    local body2c3=g.parse("6bo$b6o$o$o2b6o$obo6bo$obo2b5o$b2obo$4bo$4bo$3b2o!")
    local tail2c3=g.parse("5b2o$5b2o2$b6o$o5bo$o2b3o$obo$o2bo$b2o!")
    local wire2c3=g.join(g.transform(head2c3,625,388), g.transform(tail2c3,2143,1905))
    -- 251 body segments, first one at (631, 395)
    for i=631,2142,6 do wire2c3=g.join(wire2c3,g.transform(body2c3,i,i-236)) end 
    local receiver2c3=g.parse([[208bo$207bobo$208bo3$213b2o$188b2o23b2o$189bo31b2o$189bobo29bo$190b2o
    ...
    local inserter2c3=g.parse([[51bo$49b3o$23b2o23bo$24bo23b2o$24bobo$25b2o2b2o37bo$29b2o35b3o15bo9bo$
    ...
    local all = g.join(g.transform(highway_robber,86,0),
          g.join(g.transform(connecting_neck,195,262),
          g.join(g.transform(transmitter2c3,347,219),
          g.join(wire2c3,
          g.join(g.transform(receiver2c3,2103,1763),
    ...
    -- Could instead select each key rectangle, use g.fitsel(), then unselect.
    g.setlayer(0)
    while not g.visrect( {100,100,150,175} ) do g.setmag(g.getmag()-1) end
    g.setlayer(1)
    while not g.visrect( {350,225,400,350} ) do g.setmag(g.getmag()-1) end
    g.setlayer(2)
    while not g.visrect( {2100,1750,225,300} ) do g.setmag(g.getmag()-1) end
    g.setlayer(3)
    g.fit()
    g.setlayer(4)
    while not g.visrect( {0,200,300,400} ) do g.setmag(g.getmag()-1) end
    g.update()
end
    ...
    while ch~="q" do
        if place_signal>0 then
            if ticks-last_signal>1846 then
                g.putcells(test_signal,-150,60)
                last_signal=ticks
                place_signal=place_signal-1
            end
        end
        local status_text=instr
        if place_signal>0 and run_flag==1 then
            status_text="Next signal placement in "..(1847 - ticks + last_signal).." ticks. "
            local plural="s"
            if place_signal==2 then plural="" end
            if place_signal>1 then status_text=status_text..(place_signal-1).." more signal"..plural.." requested after that. " end
        ...
        elseif ch=="h" then g.note(helpstring)
        elseif ch=="t" then g.setoption("tilelayers",1-g.getoption("tilelayers"))

        elseif ch=="=" or ch=="+" then
            if delay>.01 then delay=delay/2
            elseif delay==.01 then delay=-1
            else
                if tickstep==1 then tickstep=3
                elseif tickstep<250 then tickstep=(tickstep-1)*2+1
                end
            end
        elseif ch=="-" or ch=="_" then
            if delay==-1 then
                if tickstep==1 then delay=.01
                else
                    if tickstep==3 then tickstep=1 else tickstep=math.floor((tickstep-1)/2)+1 end
                end
            else
                if delay<1 then delay=delay*2 end
            end

        elseif ch=="space" then run_flag=0
        elseif ch=="return" then run_flag=1-run_flag
        elseif ch=="s" then
            place_signal=place_signal+1
            if run_flag==0 then run_flag=1 end
        else g.doevent(event)
        end

        -- generation and selection speed handling
        if ch=="space" or run_flag==1 then
            g.run(tickstep)
            ...
            delta_view=math.floor(ticks/viewport_speed) - math.floor(oldticks/viewport_speed)
            if delta_view ~= 0 then g.setpos(tostring(x + delta_view), tostring(y + delta_view)) end
            sel = g.getselrect()
            if #sel ~= 0 then
                x, y, w, h = table.unpack(sel)
                if math.floor(selx)~=x then selx,sely=x,y
                else
                    selx=selx+sel_speed*tickstep
                    sely=sely+sel_speed*tickstep
                    g.select( {math.floor(selx), math.floor(sely), w, h} )
                end
            else g.select({math.floor(selx), math.floor(sely), 50, 50})
            end
            ...
            if oldticks<4570 and ticks>=4570 then
                sel_speed=.666667
                -- one-time correction to catch up with the signal at high sim speeds
                g.select( {math.floor(600+(ticks-4570)*1.5), math.floor(365+(ticks-4570)*1.5), w, h} )
                end
            if selx>2125 then
                sel_speed=0
                g.select( {2125, math.floor(sely+2125-selx), w, h} )
            end
            if oldticks<4750 and ticks>=4750 then viewport_speed=1.5 end
            if oldticks<6125 and ticks>=6125 then viewport_speed=16 end
            if oldticks>=11995 then viewport_speed=99999.9 end

            -- stop automatically after the last signal passes through the device
            if oldticks-last_signal<8705 and ticks-last_signal>=8705 then run_flag=0 end
            
            if run_flag==1 and delay>0 then sleep(delay) end
