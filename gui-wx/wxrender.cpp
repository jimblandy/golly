...
// LifeViewer
double overlayminzoom = 0.0625;
double overlaymaxzoom = 32;
int aliveStart = 64;                    // new cell color index
int aliveMax = 127;                     // cell alive longest color index
int deadStart = 63;                     // cell just died color index
int deadMin = 1;                        // cell dead longest color index
...
void RefreshCellViewWithTheme(unsigned char* cellviewdata, int cellwd, int cellht, int cellx, int celly)
{
    // refresh the cellview for a 2 state pattern using LifeViewer theme
    unsigned char *cellviewptr = cellviewdata;
    unsigned char state;
    int h, w, v = 0;
    int skip;
...
void RefreshCellView(unsigned char* cellviewdata, int cellwd, int cellht, int cellx, int celly)
{
...
void PopulateOverlay(unsigned char* overlaydata, int wd, int ht,
         unsigned char* cellviewdata, int cellwd, int cellht, int cellx, int celly)
{
    // read camera configuration
    double overlayangle = curroverlay->GetCameraAngle();
    double overlayzoom = curroverlay->GetCameraZoom();
    double overlayx = curroverlay->GetCameraX();
    double overlayy = curroverlay->GetCameraY();
    int layers = curroverlay->GetCameraLayers();
    double layerdepth = curroverlay->GetCameraLayerDepth() / 2 + 1;

    // convert zoom 0..1 to actual zoom
    double zoom = overlayminzoom * pow(overlaymaxzoom / overlayminzoom, overlayzoom);

    // check for color theme
    int theme = curroverlay->GetTheme();

    // check if cell view needs to refresh
    bool dirty = curroverlay->CellViewNeedsRefresh();

    // check pixel brightness depending on layers
    double brightness = 1;
    double brightinc = 0;

    if (theme != -1 && layers > 1 && layerdepth > 1) {
        brightness = 0.6;
        brightinc = 0.4 / (layers - 1);
    }
    ...
    if (theme != -1) {
        ...
        if (dirty) {
            RefreshCellViewWithTheme(cellviewdata, cellwd, cellht, cellx, celly);
        }
    }
    else {
        ...
        GetPatternColors();
        if (dirty) {
            RefreshCellView(cellviewdata, cellwd, cellht, cellx, celly);
        }
    }

    // compute deltas in horizontal and vertical direction based on rotation
    double dxy = sin(overlayangle / 180 * M_PI) / zoom;
    double dyy = cos(overlayangle / 180 * M_PI) / zoom;

    // compute starting position
    double sy = -((wd / 2) * (-dxy) + (ht / 2) * dyy) + overlayy;
    double sx = -((wd / 2) * dyy + (ht / 2) * dxy) + overlayx;

    unsigned char state;
    unsigned int rgba;
    unsigned int *overlayptr = (unsigned int *)overlaydata;
    double x, y;
    int ix, iy;
    int h, w;
    ...
    for (h = 0; h < ht; h++) {
        ...
        for (w = 0; w < wd; w++) {
            ...
            if (ix >= 0 && ix < cellwd && iy >= 0 && iy < cellht) {
                state = cellviewdata[cellwd * iy + ix];
                rgba = cellRGBA[state];
            }
            else {
                rgba = offRGBA;
    ...
    if (theme != -1) {
        for (int i = 1; i < layers; i++) {
            // compute transparent state
            unsigned char transparenttarget = (i * ((aliveMax + 1) / layers));
            ...
            sy = -((wd / 2) * (-dxy) + (ht / 2) * dyy) + overlayy;
            sx = -((wd / 2) * dyy + (ht / 2) * dxy) + overlayx;

            overlayptr = (unsigned int *)overlaydata;
...
void DrawOverlay()
{
    ...
    if (overlaydata) {
        ...
        // check for cellview
        unsigned char* cellviewdata = curroverlay->GetCellViewData();
        if (cellviewdata) {
            // get the size of the cellview
            int cellwd = curroverlay->GetCellViewWidth();
            int cellht = curroverlay->GetCellViewHeight();
            int cellx = curroverlay->GetCellViewX();
            int celly = curroverlay->GetCellViewY();

            // populate the overlay from the cellview
            PopulateOverlay(overlaydata, wd, ht, cellviewdata, cellwd, cellht, cellx, celly);
        }
