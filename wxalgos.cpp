...
void InitAlgorithms()
{
   ...
   for (int i = 0; i < NumAlgos(); i++) {
      ...
      if (ad->statusrgb[0] == 0 && ad->statusrgb[1] == 0 && ad->statusrgb[2] == 0) {
         ...
         for (int j=0; j<64; j++) {
            ...
            for (int k=0; k<NumAlgos(); k++) {
               if (k == i)
                  continue;
               int oldr = algoinfo[k]->statusrgb[0];
               int oldg = algoinfo[k]->statusrgb[1];
               int oldb = algoinfo[k]->statusrgb[2];
               int td = (oldr - tr) * (oldr - tr) +
                        (oldg - tg) * (oldg - tg) +
                        (oldb - tb) * (oldb - tb);
               if (td < md)
                  md = td;
      ...
      ad->algorgb = new wxColor(ad->statusrgb[0], ad->statusrgb[1], ad->statusrgb[2]);
